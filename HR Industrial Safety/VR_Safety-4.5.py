...
load_and_enable_from_file("extentions.txt")
simulation_app.update()
...
class CuRoboConv(object):
    def __init__(self,
        ...
        # self._L1_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/" + Conv_Name + "/Link_1/collisions")
        # self._Collider_Off = self._L1_Prim.GetAttribute("physics:collisionEnabled").Set(False)

        self._TL_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/" + Conv_Name + "/track_link/collisions")
        self._Collider_Off = self._TL_Prim.GetAttribute("physics:collisionEnabled").Set(False)

        self._BL_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/" + Conv_Name + "/base_link/collisions")
        self._Collider_Off = self._BL_Prim.GetAttribute("physics:collisionEnabled").Set(False)

        self._L1_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/" + Conv_Name + "/Link_1/collisions")
        self._Collider_Off = self._L1_Prim.GetAttribute("physics:collisionEnabled").Set(False)

        self._L2_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/" + Conv_Name + "/Link_2/collisions")
    ...
    def attach_object_to_conv(self,
        ...
        Conv_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/Smart_Conveyor/Link_2")

        Conv_Collision_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/Smart_Conveyor/Link_2/collisions")   
...
class CuRoboRobot(object):
    def __init__(self,
        ...
        for Gripper in Gripper_List:
            # Creating Prims !
            PrimPathX = "/World/"+Gripper.RobName+"/"+Gripper.ParentLink+"/"+"GripPosition_"+Gripper.TCP_Name+"_X"
            PrimPathY = "/World/"+Gripper.RobName+"/"+Gripper.ParentLink+"/"+"GripPosition_"+Gripper.TCP_Name+"_Y"
            PrimPathZ = "/World/"+Gripper.RobName+"/"+Gripper.ParentLink+"/"+"GripPosition_"+Gripper.TCP_Name+"_Z"
            ...
            (graph_handle, list_of_nodes, _, _) = og.Controller.edit(
                ...
                {
                    keys.CREATE_NODES: [
                        ("surfX","isaacsim.robot.surface_gripper.SurfaceGripper"),
                        ("surfY","isaacsim.robot.surface_gripper.SurfaceGripper"),
                        ("surfZ","isaacsim.robot.surface_gripper.SurfaceGripper"),
                    ...
                    keys.SET_VALUES: [
                        ...
                        ("surfX.inputs:ParentRigidBody", "/World/"+Gripper.RobName+"/"+Gripper.ParentLink),
                        ("surfY.inputs:ParentRigidBody", "/World/"+Gripper.RobName+"/"+Gripper.ParentLink),
                        ("surfZ.inputs:ParentRigidBody", "/World/"+Gripper.RobName+"/"+Gripper.ParentLink),
        ...
        self._EEF_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/" + self._ROS_JS_robot_indicator + "/Link_7/collisions")
        # <!!!> Migration to 4.5 Issue <!!!>
        self._Collider_Off = self._EEF_Prim.GetAttribute("physics:collisionEnabled").Set(False)
        self._temp_world_manager._my_world.step(render=True)

        # ## Removing EndEffector's Mass
        self._temp_world_manager._stage.GetPrimAtPath("/World/" + self._ROS_JS_robot_indicator + "/Link_7").GetAttribute("physics:mass").Set(1e-20)
        self._temp_world_manager._stage.GetPrimAtPath("/World/" + self._ROS_JS_robot_indicator + "/Link_8").GetAttribute("physics:mass").Set(1e-20)

        # Neeed To Increase Joint Damping To Avoid Shaking within Robot Joints (FIXED IN URDF)
        # for i in range (1, 6):
        #     Joint_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/" + self._ROS_JS_robot_indicator + "/Link_" + str(i) + "/Joint_" + str(i+1))
    ...
    def motion_gen_update_world(self, 
        ...
        ignoring_prim_paths = [
            ...
            "/World/Smart_Conveyor/base_link/visuals",
            "/World/Smart_Conveyor/track_link/visuals",
            "/World/Smart_Conveyor/Link_1/visuals",
            "/World/Smart_Conveyor/Link_2/visuals",
            # Other Robot's Visual Prims
            "/World/"+Rob_name+"/base_link/visuals",
            "/World/"+Rob_name+"/Link_1/visuals",
            "/World/"+Rob_name+"/Link_2/visuals",
            "/World/"+Rob_name+"/Link_3/visuals",
            "/World/"+Rob_name+"/Link_4/visuals",
            "/World/"+Rob_name+"/Link_5/visuals",
            "/World/"+Rob_name+"/Link_6/visuals",
            "/World/"+Rob_name+"/Link_7/visuals",
            "/World/"+Rob_name+"/Link_8/visuals",
        ...
        if Removing_Prim_Paths != None:
            for prim in Removing_Prim_Paths:
                ignoring_prim_paths.extend(["/World/"+prim])
            # We Do this Whenever we want to remove anything from the scene!

            # Alternative Method
            # self._motion_gen.world_model.remove_obstacle("/World/Smart_Conveyor/Link_1/collisions")
            # self._motion_gen.world_model.remove_obstacle("/World/Smart_Conveyor/Link_2/collisions")
    ...
    def free_TCP_movement(self, 
        ...
        object=dc.get_rigid_body("/World/"+self._ROS_JS_robot_indicator+"/"+moving_tcp)
                    ...
                    if Show_Sphere == True and self._temp_world_manager._my_world.current_time_step_index % 2 == 0:
                        ...
                        for si, s in enumerate(sph_list[0]):
                            ...
                            sp = sphere.VisualSphere(
                                prim_path="/World/curobo/"+self._ROS_JS_robot_indicator+"/sph" + str(si),
...
# Visualizing Spheres
        ...
        #                         s.position[2]]
        #     sp = sphere.VisualSphere(
        #         prim_path="/World/curobo/AttachedObj" + "/sph_obj" + str(si),
        #         position=np.ravel(updated_s_pose),
        #         radius=float(s.radius),
        #         color=np.array([0, 0.8, 0.2]),
        #     )
    ...
    def disable_eef_collider(self):
        Obj_Prim = self._temp_world_manager._stage.GetPrimAtPath("/World/" + self._ROS_JS_robot_indicator + "/Link_7/collisions")
        self._temp_world_manager._my_world.step(render=True)
