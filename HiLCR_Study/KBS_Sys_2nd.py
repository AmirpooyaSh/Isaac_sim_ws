...
def Nail_and_Release_Vertical_Element(
    ...
    Parameters:
        ...
        Is_Held (bool): If True, there were no hold by Robot 2 (e.g., Placing Top Cripple), so there will be no Release.
    Procedure:
...
def Drop_Short_Vertical_Element_With_Tangent_to_an_Element(
    ...
    """
    Position and drop a less than 8ft (2.4384 m) vertical element onto the smart conveyor by Robot 2, allowing it to fall
...
def Place_8ft_Vertical_Element_On_Smart_Conveyor_by_Rob1_Suction(el_name: str = None,
    ...
    Brief Description:
        ...
        This functionality is used for placing studs on their wider surface (such as L/U connection).
    Parameters:
        el_name (str): Name of the element to place.
        X (float): X value of element's position from the top left corner of the building panel.
        Y (float): Y value of element's position from the top left corner of the building panel.
...
def Complementary_Nail_Operation(push_to_nail: float = PUSH_TO_NAIL_OFFSET,
    ...
    '''
    Brief Description:
        Performs complementary nailing operations for each recorded nail pose on the smart conveyor once the the structural placements are done,
...
def main():
    ...
    while simulation_app.is_running():
        ...
        ###############################################################################
        #  Wall-panel assembly script generated for the supplied “elements” list.     #
        #  The sequence strictly follows the framing procedure and uses ONLY the      #
        #  provided robotic-station functions (no helper routines added).             #
        ###############################################################################

        # ---- Constants already defined elsewhere in the station code -------------
        # PUSH_TO_NAIL_OFFSET
        # PUSH_TO_NAIL_OFFSET_TANGENT
        # --------------------------------------------------------------------------

        # --------------------------------------------------------------------------
        # 1.  TOP PLATE  (full-length horizontal member – first element installed)
        # --------------------------------------------------------------------------
        ...
        Place_Long_Element_On_Smart_Conveyor_by_Rob1_Gripper(
                                                "Top_Plate", 0.02, 1.8288, 3.6576, 0.1016)

        # --------------------------------------------------------------------------
        # DOOR OPENING  (left-to-right bay sequence)
        # --------------------------------------------------------------------------
        # Left King Stud
        Pick_8ft_Element_From_Sloped_Table      ("L_King_1", 2.4384, 0.04, 0.1016)
        L_King_1_SS = Place_and_Hold_8ft_Element_On_Smart_Conveyor(
                                                1.2592, 0.72, 2.4384, 0.1016)
        Nail_and_Release_Vertical_Element       ("L_King_1", 1.2592, PUSH_TO_NAIL_OFFSET,
                                                2.4384, 0.1016, L_King_1_SS)

        # Right King Stud
        Pick_8ft_Element_From_Sloped_Table      ("R_King_1", 2.4384, 0.04, 0.1016)
        R_King_1_SS = Place_and_Hold_8ft_Element_On_Smart_Conveyor(
                                                1.2592, 1.68, 2.4384, 0.1016)
        Nail_and_Release_Vertical_Element       ("R_King_1", 1.2592, PUSH_TO_NAIL_OFFSET,
                                                2.4384, 0.1016, R_King_1_SS)

        # Left Jack Stud
        Drag_Stud                               ("L_Jack_1", [2.0, 0.04, 0.1016])
        Pick_Short_Element_From_Mat_Supply      ("L_Jack_1", 2.0, 0.04, 0.1016)
        L_Jack_1_SS = Drop_Short_Vertical_Element_With_Tangent_to_an_Element(
                                                "L_Jack_1", 1.4784, 0.76,
                                                2.0, 0.1016,  True)
        ...
        R_Jack_1_SS = Drop_Short_Vertical_Element_With_Tangent_to_an_Element(
                                                "R_Jack_1", 1.4784, 1.64,
                                                2.0, 0.1016,  False)
        Nail_Vertical_Element_With_Tangent_to_an_Element(
                                                PUSH_TO_NAIL_OFFSET_TANGENT,
                                                [1.4784, 1.64, 0.0],
        ...
        TS_1_CurConv = Place_Short_Horizontal_Element_On_Smart_Conveyor_by_Rob1_Gripper(
                                                "TS_1", 0.4584, 1.2, 0.92, 0.1016)
        Nail_Short_Horizontal_Element_by_Rob1_NailGun(
                                                PUSH_TO_NAIL_OFFSET_TANGENT,
                                                [0.4584, 1.2, 0.0],
                                                [0.92, 0.04, 0.1016],
                                                TS_1_CurConv)

        # Top Cripple Studs (3 pcs, left-to-right)
        for idx, (tc_name, y_loc) in enumerate(
                [("TC_1", 1.2), ("TC_2", 0.98), ("TC_3", 1.42)], start=1):
            ...
            locals()[f"{tc_name}_SS"] = Place_Short_Vertical_Element_On_Smart_Conveyor_by_Rob1_Gripper(
                                                tc_name, 0.2392, y_loc,
                                                0.3984, 0.1016)
            Nail_and_Release_Vertical_Element (tc_name, 0.2392,
                                                PUSH_TO_NAIL_OFFSET,
                                                0.3984, 0.1016,
                                                locals()[f"{tc_name}_SS"], True)
        ...
        L_King_2_SS = Place_and_Hold_8ft_Element_On_Smart_Conveyor(
                                                1.2592, 3.1376, 2.4384, 0.1016)
        Nail_and_Release_Vertical_Element       ("L_King_2", 1.2592, PUSH_TO_NAIL_OFFSET,
                                                2.4384, 0.1016, L_King_2_SS)

        # Right King Stud
        Pick_8ft_Element_From_Sloped_Table      ("R_King_2", 2.4384, 0.04, 0.1016)
        R_King_2_SS = Place_and_Hold_8ft_Element_On_Smart_Conveyor(
                                                1.2592, 2.2376, 2.4384, 0.1016)
        Nail_and_Release_Vertical_Element       ("R_King_2", 1.2592, PUSH_TO_NAIL_OFFSET,
                                                2.4384, 0.1016, R_King_2_SS)

        # Left Jack Stud
        Drag_Stud                               ("L_Jack_2", [1.8, 0.04, 0.1016])
        Pick_Short_Element_From_Mat_Supply      ("L_Jack_2", 1.8, 0.04, 0.1016)
        L_Jack_2_SS = Drop_Short_Vertical_Element_With_Tangent_to_an_Element(
                                                "L_Jack_2", 1.5784, 2.2776,
                                                1.8, 0.1016,  True)
        Nail_Vertical_Element_With_Tangent_to_an_Element(
                                                PUSH_TO_NAIL_OFFSET_TANGENT,
                                                [1.5784, 2.2776, 0.0],
        ...
        R_Jack_2_SS = Drop_Short_Vertical_Element_With_Tangent_to_an_Element(
                                                "R_Jack_2", 1.5784, 3.0976,
                                                1.8, 0.1016,  False)
        Nail_Vertical_Element_With_Tangent_to_an_Element(
                                                PUSH_TO_NAIL_OFFSET_TANGENT,
                                                [1.5784, 3.0976, 0.0],
        ...
        # Header – Bear Loading plank 1
        Pick_2x10_Header                        (0.5514, 0.86)
        Cut_2x10_Header                         (0.86, 0.0508, 0.254)
        BL_1_CurConv = Place_2x10_Header        (0.5514, 2.6876, 0.0254,
                                                0.86, 0.0508, 0.254)
        Nail_2x10_Header                        (PUSH_TO_NAIL_OFFSET_TANGENT*2.5,
                                                [0.5514, 2.6876, 0.0254],
                                                [0.86, 0.0508, 0.254],
                                                BL_1_CurConv)

        # Header – Bear Loading plank 2 (stacked)
        Pick_2x10_Header                        (0.5514, 0.96)
        Cut_2x10_Header                         (0.86, 0.0508, 0.254)
        BL_2_CurConv = Place_2x10_Header        (0.5514, 2.6876, 0.0762,
                                                0.86, 0.0508, 0.254)
        Nail_2x10_Header                        (PUSH_TO_NAIL_OFFSET_TANGENT*2.5,
                                                [0.5514, 2.6876, 0.0762],
                                                [0.86, 0.0508, 0.254],
                                                BL_2_CurConv)
        ...
        BS_1_CurConv = Place_Short_Horizontal_Element_On_Smart_Conveyor_by_Rob2_Gripper(
                                                "BS_1", 1.8984, 2.6876, 0.78, 0.1016)
        Nail_Short_Horizontal_Element_by_Rob2_NailGun(
                                                PUSH_TO_NAIL_OFFSET_TANGENT,
                                                [1.8984, 2.6876, 0.0],
                                                [0.78, 0.04, 0.1016],
                                                BS_1_CurConv)

        # Lower Cripple Studs (2 pcs under sill)
        Drag_Stud                               ("LC_1", [0.56, 0.04, 0.1016])
        Pick_Short_Element_From_Mat_Supply      ("LC_1", 0.56, 0.04, 0.1016)
        Place_Short_Vertical_Element_On_Smart_Conveyor_by_Rob2_Gripper(
                                                "LC_1", 2.1984, 2.5376, 0.56, 0.1016)

        Drag_Stud                               ("LC_2", [0.56, 0.04, 0.1016])
        Pick_Short_Element_From_Mat_Supply      ("LC_2", 0.56, 0.04, 0.1016)
        Place_Short_Vertical_Element_On_Smart_Conveyor_by_Rob2_Gripper(
                                                "LC_2", 2.1984, 2.8376, 0.56, 0.1016)

        # --------------------------------------------------------------------------
        # STANDARD STUD BAY (first stud to right of opening)
        # --------------------------------------------------------------------------
        Pick_8ft_Element_From_Sloped_Table      ("F_Stud", 2.4384, 0.04, 0.1016)
        F_Stud_SS = Place_and_Hold_8ft_Element_On_Smart_Conveyor(
                                                1.2592, 0.02, 2.4384, 0.1016)
        Nail_and_Release_Vertical_Element       ("F_Stud", 1.2592, PUSH_TO_NAIL_OFFSET,
                                                2.4384, 0.1016, F_Stud_SS)

        # L/U Stud (installed immediately after first stud)
        ...
        Place_8ft_Vertical_Element_On_Smart_Conveyor_by_Rob1_Suction(
                                                "LU_Stud", 1.2592, 0.0908, 0.02,
                                                2.4384, 0.04, 0.1016)

        # Repetitive full-height studs (three more, left-to-right)
        for stud_name, y_val in [("R_1_Stud", 0.50),
                                ("R_2_Stud", 1.9588),
                                ("R_3_Stud", 3.6376)]:
            Pick_8ft_Element_From_Sloped_Table  (stud_name, 2.4384, 0.04, 0.1016)
            ss_var = Place_and_Hold_8ft_Element_On_Smart_Conveyor(
                                                1.2592, y_val,
                                                2.4384, 0.1016)
            Nail_and_Release_Vertical_Element   (stud_name, 1.2592, PUSH_TO_NAIL_OFFSET,
                                                2.4384, 0.1016, ss_var)

        # --------------------------------------------------------------------------
        # BOTTOM PLATE  (installed after all vertical members are secure)
        # --------------------------------------------------------------------------
        Pick_Long_Element_From_Mat_Supply       ("Bottom_Plate", 3.6576, 0.04, 0.1016)
        Place_Long_Element_On_Smart_Conveyor_by_Rob2_Gripper(
                                                "Bottom_Plate", 2.4984, 1.8288,
                                                3.6576, 0.1016)

        # --------------------------------------------------------------------------
        # COMPLEMENTARY NAIL PASS  (finishes remaining nailing tasks)
        # --------------------------------------------------------------------------
